...
FILTER_ADDRESS = "66CA6B53-55A5-6664-28DF-6929668E225A"
    ...
    async def monitor_loop(self):
        while self._status == Status.RUNNING:
            try:
                ...
                if is_last_packet:
                    parsed_json_data = self.parseJson()
...
async def main():
    filter_client = FilterClient(address=FILTER_ADDRESS, auth_token=AUTH_TOKEN)
    try:
        await filter_client.connect()
        await filter_client.monitor_loop()
    finally:
        await filter_client.clean()
